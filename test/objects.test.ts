...
function fixture(): World {
  return {
    sessionId: "one",
    entities: [
      { ref: pet.ref("one", "p"), value: { hp: 5 } },
      { ref: mark.ref("one", "m"), value: { stacks: 1 } },
    ],
    relations: [
      {
        id: "link",
...
test("object types, session identity, field schema and relation cardinality are enforced", () => {
  const w = parseWorld(fixture());
  const q = new WorldQuery(w);
  ...
  const writer = new WorldEditor(
    w,
    { objects: ["mark"], relations: ["attached"] },
    [attached],
  );
  assert.throws(() => writer.set(pet, pet.ref("one", "p"), { hp: 8 }));
  const read = q.get(pet, pet.ref("one", "p"));
  read.hp = 0;
...
test("target deletion cascades attachments, retires identities and denies resurrection", () => {
  ...
  const editor = new WorldEditor(
    w,
    { objects: ["pet", "mark"], relations: ["attached"] },
    [attached],
  );
  editor.remove(pet.ref("one", "p"));
  assert.equal(parseWorld(w).entities.length, 0);
...
test("relation cycles and restricted deletion reject", () => {
  ...
  const world: World = {
    sessionId: "one",
    entities: [
      { ref: pet.ref("one", "a"), value: { hp: 1 } },
      { ref: pet.ref("one", "b"), value: { hp: 2 } },
    ],
    relations: [
      {
        id: "ab",
  ...
  const validate = worldParser([pet], [parent]);
  validate(world);
  assert.throws(() =>
    new WorldEditor(world, { objects: ["pet"], relations: ["parent"] }, [
      parent,
    ]).remove(pet.ref("one", "b")),
  );
