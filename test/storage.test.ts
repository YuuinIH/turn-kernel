...
const game: GameDefinition<number, number, number> = {
  ruleset: "count/1",
  parseState: parse.integer,
  parseCommand: parse.integer,
...
async function contract(store: SessionStore, expire: () => Promise<void>) {
  ...
  await assert.rejects(() =>
    store.create(createSession(game, "one", 0).snapshot()),
  );
  const a = await store.claim("one", "worker-a", 10000);
  await assert.rejects(() => store.claim("one", "worker-b", 10000));
  const candidate = (requestId: string, value: number): Commit => ({
    sessionId: "one",
    lease: a,
    expectedRevision: 0,
    next: {
    ...
    receipt: {
      ...
      result: { ok: true, revision: 1, facts: [value] },
    },
  });
  // Renew to avoid test process timing affecting the race portion.
  await store.claim("one", "worker-a", 10000);
  const outcomes = await Promise.all([
    store.commit(candidate("first", 1)),
    store.commit(candidate("second", 2)),
  ]);
  assert.equal(outcomes.filter((v) => v === "committed").length, 1);
  assert.equal(outcomes.filter((v) => v === "stale").length, 1);
  const winning =
  ...
  assert.equal(
    await store.commit({
      ...winning,
      receipt: { ...winning.receipt, fingerprint: "changed" },
    }),
    "request-conflict",
  );
  ...
  await expire();
  const b = await store.claim("one", "worker-b", 10000);
  assert.ok(b.fence > a.fence);
  const next = {
    ...winning,
    expectedRevision: 1,
    next: { ...winning.next, revision: 2 },
    receipt: { requestId: "third", fingerprint: "third", result: {} },
  };
...
test("memory store implements CAS, receipts and fencing contract", async () => {
  let now = 0;
  await contract(new MemoryStore(() => now), async () => {
    now += 20000;
  });
});
test("real Redis implements the same CAS, receipts and fencing contract", async (t) => {
  try {
    ...
    if (process.env.REQUIRE_REDIS === "1")
      throw Error("Redis binaries required");
    t.skip("Install Redis or run CI with REQUIRE_REDIS=1");
    return;
  }
  const dir = await mkdtemp(join(tmpdir(), "turn-kernel-redis-"));
  ...
  try {
    ...
    for (let i = 0; i < 100; i += 1) {
      try {
        await access(socket);
        await exec("redis-cli", ["-s", socket, "PING"]);
        ready = true;
        break;
      } catch {
        await delay(20);
    ...
    const evaluate = async (
      ...
      const { stdout } = await exec("redis-cli", [
        "-s",
        socket,
        "--json",
        "EVAL",
...
test("durable session recovers lost acknowledgement, retries once and survives ownership transfer", async () => {
  ...
  let fail = true;
  const uncertain: SessionStore = {
    create: (s) => memory.create(s),
    claim: (s, o, ttl) => memory.claim(s, o, ttl),
    load: (s) => memory.load(s),
    receipt: (s, r) => memory.receipt(s, r),
    commit: async (c) => {
      const result = await memory.commit(c);
      if (fail) {
        fail = false;
  ...
  const a = await openDurableSession(
    game,
    uncertain,
    "one",
    "a",
  ...
  const first = await a.dispatch(4, 0, "r1");
