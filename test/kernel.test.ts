...
test("commit, reject failed partial write, reject duplicate, restore waiting state", () => {
  ...
  assert.equal(
    session.submit({ sessionId: "other", revision: 1, command: { amount: 2 } })
      .ok,
    false,
  );
  assert.deepEqual(
    restoreSession(counter, JSON.parse(JSON.stringify(saved))).snapshot(),
    saved,
  );
...
test("invalid candidate, invalid command and invalid snapshot are rejected without writes", () => {
  ...
  for (const command of [
    ...
    assert.equal(
      session.submit({ sessionId: "one", revision: 0, command }).ok,
      false,
    );
    assert.deepEqual(session.snapshot(), saved);
  }
  ...
  assert.throws(() =>
    restoreSession(counter, { ...saved, state: { count: -1 } }),
  );
