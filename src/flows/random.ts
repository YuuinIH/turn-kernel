...
export interface RandomResult { state: number; value: number }
...
export function nextRandom(state: number, exclusiveMax: number): RandomResult {
  integer(state, 0, 0xffffffff); integer(exclusiveMax, 1, 0x100000000);
  ...
  let next = state;
  do { next = (Math.imul(next, 1664525) + 1013904223) >>> 0; } while (next >= limit);
