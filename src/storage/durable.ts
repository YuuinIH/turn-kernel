...
export type DurableResult<F> =
  | Submission<F>
  | {
      ok: false;
      code: "request-conflict" | "not-owner" | "storage-uncertain";
      reason: string;
    };
function canonical(value: unknown): string {
  if (Array.isArray(value))
    return `[${value.map((v: unknown) => canonical(v)).join(",")}]`;
  if (record(value))
  ...
  if (encoded === undefined) throw Error("Not JSON");
  return encoded;
}
export async function openDurableSession<S, C, F>(
  ...
  async function submit(input: unknown): Promise<DurableResult<F>> {
    ...
    try {
      ...
      return {
        ok: false,
        code: "invalid-input",
        reason: "Invalid durable request",
      };
    }
    ...
    try {
      ...
      if (outcome === "not-owner")
        return {
          ok: false,
          code: "not-owner",
          reason: "Lease expired or replaced",
        };
      return { ok: false, code: "stale-revision", reason: "Concurrent commit" };
    } catch {
      try {
        const receipt = await store.receipt(sessionId, requestId);
        if (receipt) return decode(receipt, fingerprint);
      } catch {
        /* preserve uncertainty */
      }
      return {
        ok: false,
        code: "storage-uncertain",
        reason: "Retry with the same request ID and payload",
      };
    }
  }
  return {
    submit,
    dispatch: (command: C, revision: number, requestId: string) =>
      submit({ sessionId, revision, requestId, command }),
    snapshot: async (): Promise<Snapshot<S>> =>
      restoreSession(game, (await store.load(sessionId)).snapshot).snapshot(),
    renew: async (): Promise<void> => {
      const renewed = await store.claim(sessionId, owner, leaseMs);
      if (renewed.fence !== lease.fence)
        throw Error("Lease lost; reopen the session");
    },
  };
}
