...
export class RedisStore implements SessionStore {
  constructor(
    ...
    private readonly prefix = "turn-kernel",
  ) {}
  #key(id: string): string {
    return `${this.prefix}:{${encodeURIComponent(text(id))}}:session`;
  }
  #run(id: string, ...args: string[]): Promise<unknown> {
    return this.evaluate(SESSION_SCRIPT, [this.#key(id)], args);
  }
  ...
  async claim(id: string, owner: string, leaseMs: number): Promise<Lease> {
    ...
    const fence = integer(
      Number(await this.#run(id, "claim", owner, String(leaseMs))),
      1,
    );
    return { owner, fence };
  }
  async load(id: string): Promise<StoredSession> {
    ...
    return {
      snapshot: parsed,
      fence: integer(Number(fence)),
      owner,
      leaseUntil: integer(Number(expiry)),
    };
  }
  ...
  async commit(input: Commit): Promise<CommitResult> {
    ...
    const result = await this.#run(
      ...
      "commit",
      c.lease.owner,
      String(c.lease.fence),
      String(c.expectedRevision),
      c.receipt.requestId,
      c.receipt.fingerprint,
      JSON.stringify(c.next),
      JSON.stringify(c.receipt),
      c.next.ruleset,
