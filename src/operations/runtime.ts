...
export function defineOperation<S, I, F>(
  definition: OperationDefinition<S, I, F>,
) {
  const id = text(definition.id);
  const version = text(definition.version);
  const parse = definition.parse;
  const execute = definition.execute;
  const authorize = definition.authorize;
  const operation: Operation<S, F> = Object.freeze({
    id,
    version,
  ...
  return Object.freeze({
    operation,
    request: (input: I): OperationRequest => ({
      operation: id,
      version,
      input: detached(parse(detached(input))),
    }),
  });
}
function ordered<T extends { id: string; order: number }>(
  rules: readonly T[],
): T[] {
  if (new Set(rules.map((r) => r.id)).size !== rules.length)
    throw Error("Duplicate rule ID");
...
export class OperationRuntime<S, F> {
  ...
  constructor(
    parse: Parser<S>,
    operations: readonly Operation<S, F>[],
    before: readonly BeforeRule<S>[] = [],
    reactions: readonly ReactionRule<S, F>[] = [],
  ) {
    this.#parse = parse;
    this.#operations = new Map(operations.map((o) => [o.id, o]));
    if (this.#operations.size !== operations.length)
      throw Error("Duplicate operation");
  ...
  execute(
    ...
    while (queue.length) {
      ...
      if (!operation || operation.version !== request.version)
        throw Error("Unknown operation or version");
      ...
      for (const fact of result.facts) {
        ...
        for (const rule of this.#reactions)
          queue.push(...detached(rule.react(detached(state), detached(fact))));
      }
    }
    return { state, facts };
  }
...
export function defineBehavior<S, I>(
  id: string,
  version: string,
  parse: Parser<I>,
  allowed: readonly { id: string; version: string }[],
  plan: (query: () => S, input: I) => readonly OperationRequest[],
) {
  const permissions = new Set(
    allowed.map((o) => JSON.stringify([o.id, o.version])),
  );
  return Object.freeze({
    id: text(id),
    version: text(version),
    parseInput: (input: unknown) => detached(parse(detached(input))),
    plan(state: S, input: unknown): readonly OperationRequest[] {
      const source = detached(state);
      const requests = detached(
        plan(() => detached(source), detached(parse(detached(input)))),
      );
      if (
        requests.some(
          (r) => !permissions.has(JSON.stringify([r.operation, r.version])),
        )
      )
        throw Error("Behavior operation denied");
      return requests;
    },
  });
}
